...
func JSONEncoder(body []byte, dto interface{}) {
	...
	if encodeError != nil {
		panic(
			NewError(constants.StatusBadRequest, constants.BAD_REQUEST, encodeError.Error()),
		)
	}
}
...
func JSONDecoder(body interface{}) []byte {
	...
	if decodeError != nil {
		panic(
			NewError(constants.StatusBadRequest, constants.BAD_REQUEST, decodeError.Error()),
		)
	}
